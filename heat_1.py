    ...
    for char in sequence.upper():
        if char in morse_code:
            for i, symbol in enumerate(morse_code[char]):
                if symbol == '.':
                    heat_pulses.extend([heat_dot] * duration_dot)
                elif symbol == '-':
                    heat_pulses.extend([heat_dash] * duration_dash)
...
for step in range(len(heat_sequence)):
    if heat_sequence[step]:
        heat_grid[source_position] = heat_sequence[step]
    heat_grid = diffuse_heat(heat_grid, source_position)
    heat_grid = cool_down(heat_grid)
